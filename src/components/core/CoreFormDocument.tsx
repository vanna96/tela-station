...
> {
  ...
  protected handlerConfirmItem(data: any[]) {
    ...
    const newItems = data.filter(
      (newItem) => !oldItems.find((e) => e?.ItemCode === newItem.ItemCode)
  ...
  protected handlerChange(key: string, value: any) {
    ...
    switch (key) {
      case "vendor":
        ...
        temps["ContactPersonCode"] =
          vendor.contactEmployee!.length > 0
            ? vendor.contactEmployee![0].id
            : undefined;
      ...
      case "DocDiscountPercent":
        ...
        temps["DocDiscountPrice"] =
          discount >= 100 ? 0 : (DocTotalBeforeDiscount * value) / 100;
  ...
  findTotalVatRate(temps: any) {
    let totalVatRate = temps.items?.reduce(
      (prev: any, current: any) => prev + (current?.vatRate ?? 0),
      0
    );
    const total = temps["DocTotalBeforeDiscount"] - temps["DocDiscountPrice"];
    totalVatRate =
      ((temps["DocDiscountPrice"] === 0
        ? temps.docTotalBeforeDiscount
        : temps["DocDiscountPrice"]) *
        totalVatRate) /
      100;
