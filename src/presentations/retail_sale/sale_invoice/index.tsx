...
export default function InvoiceLists() {
        ...
        Cell: (cell: any) => (
          <div className="flex space-x-2">
            <button
              ...
              onClick={() => {
                route("/sale/invoice/" + cell.row.original.DocEntry, {
  ...
  const Count: any = useQuery({
    ...
    queryFn: async () => {
      const response: any = await request(
        "GET",
        `${url}/Invoices/$count?$select=DocNum${filter}`
      )
  ...
  const { data, isLoading, refetch, isFetching }: any = useQuery({
    ...
    queryFn: async () => {
      const response: any = await request(
        "GET",
        `${url}/Invoices?$top=${pagination.pageSize}&$skip=${
    ...
    <>
      ...
      <div className="w-full h-full px-6 py-2 flex flex-col gap-1 relative">
        <div className="flex pr-2  rounded-lg justify-between items-center z-10 top-0 w-full  py-2">
          <h3 className="text-base 2xl:text-base xl:text-base ">Sale / Invoices</h3>
        </div>
