...
class Form extends CoreFormDocument {
  ...
  async onInit() {
    ...
    //   if (!data) {
    //     await request("GET", `ReturnRequest(${id})`)
    //       .then((res: any) => {this.props.query.set("pa-id-" + id, res?.data)})
    //       .catch((err: any) => console.log(err))
    //   }

    //   data["Status"] = data?.Status === "O" ? "F" : data?.Status
    //   disables["StartDate"] =
    //     data?.Status?.includes("A") || data?.Status?.includes("T")
    //   disables["EndDate"] =
    //     data?.Status?.includes("A") || data?.Status?.includes("T")
    //   disables["PaymentMethod"] =
    //     data?.Status?.includes("A") || data?.Status?.includes("T")
    //   disables["PaymentMethod"] =
    //     data?.Status?.includes("A") || data?.Status?.includes("T")
    //   disables["ShippingType"] =
    //     data?.Status?.includes("A") || data?.Status?.includes("T")
    //   disables["PaymentTermType"] =
    //     data?.Status?.includes("A") || data?.Status?.includes("T")
    //   disables["Status"] = data?.Status?.includes("T")
    //   disables["AgreementType"] =
    //     data?.Status?.includes("A") || data?.Status?.includes("T")
    //   disables["Projects"] =
    //     data?.Status?.includes("A") || data?.Status?.includes("T")
    //   disables["TerminateDate"] = true
    //   disables["DocumentLine"] =
    //     data?.Status?.includes("A") || data?.Status?.includes("T")
  ...
  async handlerSubmit(event: any) {
    ...
    try {
      ...
      // items
      const DocumentLines = getItem(data?.Items || [])
      ...
      if (id) {
        return await request("PATCH", `/ReturnRequest(${id})`, payloads)
          .then((res: any) =>
      ...
      await request("POST", "/ReturnRequest", payloads)
        .then((res: any) =>
