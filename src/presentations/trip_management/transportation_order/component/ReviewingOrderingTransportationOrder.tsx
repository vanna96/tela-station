...
export default function ReviewingOrderingTransportationOrder(props: any) {
  ...
  const documents: any[] = useMemo(() => {
    ...
    const data: any[] = [];
    for (let index = 0; index < (props?.watch('TL_TO_ORDERCollection') ?? []).length; index++) {
      const value = props?.watch('TL_TO_ORDERCollection')[index];
  ...
  const undOrdering: any[] = useMemo(() => {
    return documents.filter((e) => e?.U_Order <= 0).sort((a, b) => a?.U_Order - b?.U_Order);
  }, [documents])
    ...
    <>
      <div className="rounded-lg shadow-sm  border p-6 m-3 px-8 h-full">
        <div className="font-medium text-lg flex gap-x-3 items-center mb-5 pb-1 gap2">
          ...
          <div role="button" className="text-[12px] border px-2 rounded hover:bg-gray-100">Change Ordering</div>
        </div>

        {orderings.length === documents.length && <div className="w-full min-w-[30rem] whitespace-nowrap overflow-auto">
              ...
              {orderings.map((e: any, index: number) => {
                return <React.Fragment key={shortid.generate()}>
                  <tr className={`text-[13px] border ${e?.U_DocType === 'S' ? 'bg-gray-200' : ''}`}>
                    ...
                    <td className="p-2 px-3">{index === 0 ? props?.watch('U_BaseStation') : documents[index]?.U_ShipToCode}</td>
        ...
        {undOrdering.length !== 0 && <div className="flex gap-5">
