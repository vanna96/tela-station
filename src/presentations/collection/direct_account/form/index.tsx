...
class Form extends CoreFormDocument {
  ...
  outGoing: any = async (data: any) =>
    await request(
      "GET",
      `/view.svc/${
        data?.DocType === "rSupplier"
          ? "Biz_OutGoing_A_P_B1SLQuery"
          : "Biz_OutGoing_A_R_B1SLQuery"
      }?$filter=DocStatus eq 'O'  and BPCode eq '${data?.CardCode}'`
    )
      .then((res: any) => {
        const results = res.data?.value?.sort(
    ...
    React.useEffect(() => {
      if (this.state?.CardCode) {
        this.setState({
          ...this.state,
          ContentLoading: true,
          JournalMemo: `Outgoing Payments - ${this.state?.CardCode}`,
        })
        this.outGoing(this.state)
      }
    }, [this.state?.CardCode])
