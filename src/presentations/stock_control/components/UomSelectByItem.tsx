...
const getUOMGroup = async (item?: string | undefined) => {
  ...
  const response: any = await request(
    "GET",
    `/Items('${item}')?$select=UoMGroupEntry`
  );
...
export default function UomSelectByItem(props: UomSelectProp) {
  ...
  const group = useQuery({
    queryKey: [`uom_group_lists_${props.item}`],
    queryFn: () => getUOMGroup(props.item),
  });
  const uomList = useQuery({
    queryKey: ["uom_lists"],
    queryFn: () =>
      request("GET", "UnitOfMeasurements?$select=Code,Name,AbsEntry"),
  });
  ...
  const data: any[] = useMemo(() => {
    ...
    const uomGroups: number[] = group.data?.UoMGroupDefinitionCollection?.map(
      (e: any) => e.AlternateUoM
    );
    return (uomList.data as any).data.value?.filter((e: any) =>
      uomGroups?.includes(e.AbsEntry)
    );
